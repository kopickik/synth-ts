import { EventBus, NoteOnEvent, NoteOffEvent } from "./event-bus";
...
export class SynthKey {
  ...
  private constructor(note: string, index: number) {
    ...
    this.frequency =
      440 *
      Math.pow(2, (KeyDefinition.scale.indexOf(this.note) - 10) / 12) *
      (this.octave ? 2 : 1);
  }
...
export class KeyDefinition {
  ...
  private static get notes(): Array<string> {
    return [
      "C",
      "C#",
      "D",
  ...
  public static get scale(): Array<string> {
    return KeyDefinition.notes.concat(KeyDefinition.notes);
  }

  public get keys(): Array<HTMLButtonElement> {
    return KeyDefinition.scale.map((note, i) =>
      SynthKey.create(note, i, this.bus)
    );
  }
