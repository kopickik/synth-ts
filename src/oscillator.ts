import { WAVEFORM, CHANNEL, SynthComponent } from "./synth-component";
...
export class Oscillator extends SynthComponent {
  ...
  public play(freq: number) {
    ...
    this.node.start();
    this.clamp([0, 0.4, 1], 0.5);
  }

  public stop() {
    this.playing = false;
    this.clamp([1, 0.6, 0], 0.08);
    this.node.stop(this.ctx.currentTime + 0.1);
  }
}
