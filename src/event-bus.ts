...
import { CHANNEL, SynthComponent } from "./synth-component";
...
export class EventBus {
  ...
  listen(listener: Function): number {
    return this.listeners.push(listener);
  }

  fire<T extends SynthEvent>(event: T) {
    this.listeners.forEach((listener) => listener.call(listener, event));
  }
}
